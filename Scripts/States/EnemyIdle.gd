...
func randomize_parameters():
	...
	var distance_to_wander = idle_movement_speed * wander_time_duration * roundi(new_direction)
...
func _on_idle_time_timeout():
	...
	if !initial_idle_position:
		initial_idle_position = state_machine.parent_node.position.x
